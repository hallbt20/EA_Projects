...
End Sub
    ...
    Sub CreatePivotTables()
        ...
        wsSummary.Range("A32").Value = "FYTD Wins/Losses"
        wsSummary.Range("A32").Font.Bold = True
        ...
        Set pt2 = ptCache2.CreatePivotTable( _
            TableDestination:=wsSummary.Range("A33"), _
    ...
    Sub CreateAdditionalPivotTable()
        ...
        Set dataRange = wsClosed.Range("A1:AD" & wsClosed.Cells(wsClosed.Rows.Count, "A").End(xlUp).Row)
        ...
        Sheets("Originations (Wins)").Select
        ActiveSheet.PivotTables("PivotTableOriginations").PivotFields( _
            "Opportunity Originator").AutoSort xlDescending, "First Year Fees ", _
            ActiveSheet.PivotTables("PivotTableOriginations").PivotColumnAxis.PivotLines(2) _
            , 1
